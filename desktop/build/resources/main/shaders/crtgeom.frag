...
weights = (weights + scanlineWeights(uv_ratio.y, col)) / 2.0;
weights2 = (weights2 + scanlineWeights(1.0 - uv_ratio.y, col2)) / 2.0;
#endif

// Calculate the final colour of the pixel by summing the
// influence of the current and next scanlines.
vec3 rgb = col.rgb * weights.rgb + col2.rgb * weights2.rgb;
vec3 ret;
#ifdef USEGAUSSIAN
    ret = rgb * cval;
#else
    ret = vec3(rgb * cval + (1.0 - cval));
#endif
...
// Apply a gamma correction curve.
ret = inv_gamma(ret, pwr);
